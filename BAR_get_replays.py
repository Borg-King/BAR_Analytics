...
date_end = "2023-06-11" # datetime.datetime.today().strftime('%Y-%m-%d') #"2023-04-28"
# date_end seems to be upto and not including


str_game_type = "match_rating_analysis" #"pro_duels"
url_replays_store = r"https://storage.uk.cloud.ovh.net/v1/AUTH_10286efc0d334efd917d476d7183232e/BAR/demos/"
path_local_replays = r"Z:\BAR_replays_analytical\\"+str_game_type+"\\"+date_start+"_"+date_end
path_output = r"Z:\SyncedFiles\LAE_Python\BAR_Analysis\Output\\"+str_game_type
path_int = r"Z:\SyncedFiles\LAE_Python\BAR_Analysis\Intermediate\\"+str_game_type
path_replay_metadata = path_int+"\\replay_metadata"
...
with open(path_replay_metadata+r"\\replay_metadata_"+date_start+"_"+date_end+".txt","w") as f:
    f.write(get_replays_num_results)
    f.write("\n")
    f.write("num_results:"+str(num_results))

list_files_replay_data_existing = glob.glob(path_replay_metadata+r"\replay_detailed_data_*") # * means all if need specific format then *.csv
list_replay_detailed_data_existing = []
if len(list_files_replay_data_existing) > 0:
    path_latest_replay_data_existing = max(list_files_replay_data_existing, key=os.path.getctime)
    
    #list_replay_detailed_data = json.load(open(path_replay_metadata+r"\replay_detailed_data_"+date_start+"_"+date_end+".json"))
    list_replay_detailed_data_existing = json.load(open(path_latest_replay_data_existing))
...
use_existing = False
if use_existing:
    existing_pages = int(round_down(len(list_replay_detailed_data_existing),1000)/int(limit))
else:
    existing_pages = 0
...
create_folder(path_local_replays)
...
#%% Get replay metadata using IDs


if use_existing:
    list_replay_id_existing = [f["id"] for f in list_replay_detailed_data_existing]
    list_replay_detailed_data = list_replay_detailed_data_existing.copy()
else:
    list_replay_id_existing = []
    list_replay_detailed_data = []
#list_replay_detailed_data = []
for replay_data in tqdm.tqdm(list_replays_data):
    str_id = replay_data["id"]
    if str_id not in list_replay_id_existing:
        get_replay_detailed_data = url_BAR_replays_api+"/"+str_id
        dict_replay_detailed_data = requests.get(get_replay_detailed_data).json()
        list_replay_detailed_data += [dict_replay_detailed_data]
...
# list_replay_id = [f["id"] for f in list_replay_detailed_data]
# list_replay_id_unique = list(set(list_replay_id))
# list_replay_id_dups = [d for d in list_replay_id if list_replay_id.count(d)>1]

# list_replay_detailed_data_dedup = []
# list_replay_id_dedup = []
# for dict_replay in list_replay_detailed_data:
#     str_id = dict_replay["id"]
#     if str_id not in list_replay_id_dedup:
#         list_replay_id_dedup += [str_id]
#         list_replay_detailed_data_dedup += [dict_replay]
#%% Save replay metadata as json
            
with open(path_replay_metadata+r"\\replay_detailed_data_"+date_start+"_"+date_end+".json", 'w') as fout:
    json.dump(list_replay_detailed_data, fout)

#%% High rated players only filter

# list_replay_detailed_data_pro_duels = []

# rating_cut_off = 30
# if rating_cut_off:
#     for replay_detailed_data in list_replay_detailed_data[:]:
#         replay_has_unskilled_player = False
#         dict_AllyTeams = replay_detailed_data["AllyTeams"]
...
#             list_replay_detailed_data_pro_duels += [replay_detailed_data]

# with open(path_replay_metadata+r"\\replay_detailed_data_pro_duels_"+date_start+"_"+date_end+".json", 'w') as fout:
#     json.dump(list_replay_detailed_data_pro_duels, fout)

#%% Download replay files - high bandwidth!
download_full_replays = False
if download_full_replays:
    for replay_detailed_data in tqdm.tqdm(list_replay_detailed_data_pro_duels):
        replay_filename = replay_detailed_data["fileName"]
